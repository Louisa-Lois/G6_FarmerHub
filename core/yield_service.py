...
class YieldService:
    ...
    def __init__(self, model_path=None, data_path=None):
        ...
        self._rain_latest = (
            df.sort_values("year").groupby("region").rainfall_mm.last().to_dict()
        )
        ...
        self._fertility_cols = [
            "organic_matter_pct_mid",
            "total_nitrogen_pct_mid",
            "cation_exchange_capacity_mid",
        ]
        self._fertility_range = {
            c: (df[c].min(), df[c].max()) for c in self._fertility_cols
        }
    ...
    def predict(self, region, crop, year=2024, rainfall_mm=None):
        ...
        rain = self._rain_latest[region] if rainfall_mm is None else float(rainfall_mm)
        ...
        return {
            ...
            "caveat": (
                "Trained on the ten best-performing districts per crop, "
            ...
            [
                {
                    ...
                    "cation_exchange_capacity_mid": soil[
                        "cation_exchange_capacity_mid"
                    ],
                    "n_to_p_ratio": (soil["total_nitrogen_pct_mid"] / phos)
                    if phos
                    else 0.0,
...
if __name__ == "__main__":
    ...
    for r in ["ASHANTI", "NORTHERN", "VOLTA", "UPPER WEST"]:
        ...
        print(
            f"  {r:<12} {out['predicted_yield_mt_ha']:>6} Mt/Ha  "
    ...
    for mm in [800, 1200, 1600]:
        ...
        print(
            f"  {mm} mm -> {out['predicted_yield_mt_ha']} Mt/Ha "
